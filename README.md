...
* **요약:** 하드웨어 조립, 모션 제어 코드 구현, 스마트폰 카메라와 OpenCV를 연동해 객체 탐지, 로봇팔이 해당 위치로 이동하도록 물리적 좌표를 매핑하는 동적 제어 시스템 구현.
* **상세 역할:**
  ...
  * VS Code 환경에서 OpenCV를 이용해 스마트폰 카메라 영 데이터 수신, 계산된 좌표 데이터를 ESP32 마이크로컨트롤러로 전송하여 서보모터의 동적 모션 제어 달성.
